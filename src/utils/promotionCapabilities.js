...
export function normalizePromotion(promo = {}) {
  ...
  return {
    ...
    activation_mode: promo.activation_mode || 'read_only',
    can_manual_activate: promo.can_manual_activate ?? false,
...
export function promotionCountTotal(promo) {
  return Number(promo?.total_count
    ?? Number(promo?.candidate_count || 0) + Number(promo?.active_count || 0) + Number(promo?.paused_count || 0))
}

export function getPromotionTypeFilterOptions() {
  return PROMOTION_FILTER_TYPES.map((type) => ({
    label: getPromotionTypeMeta(type).label,
    value: type,
  }))
}
