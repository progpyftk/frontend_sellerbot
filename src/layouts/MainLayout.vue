<template>
  <q-layout view="hHh lpR fFf" id="inspire">
    <!-- Menu Lateral -->
    <q-drawer v-model="drawer" side="left" :width="220" :breakpoint="500" bordered class="bg-grey-1">
      <q-scroll-area class="fit">
        <q-list padding class="menu-list">
          <template v-for="(section, index) in menuSections" :key="index">
            ...
            <q-item v-for="item in section.items" :key="item.route" clickable v-ripple @click="navigateTo(item.route)"
              :to="{ name: item.route }" exact active-class="my-menu-link">
    ...
    <q-header class="bg-white text-black" elevated>
      <q-toolbar class="q-pa-none">
        ...
        <q-btn flat round size="lg" icon="mdi-menu" @click="drawer = !drawer" color="primary"></q-btn>
        ...
        <div class="user-area q-gutter-sm">
          ...
          <template v-else>
            <q-btn unelevated color="primary" label="Login" no-caps @click="redirectToLogin" />
          </template>
        </div>
      </q-toolbar>
    </q-header>
