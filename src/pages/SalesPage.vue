<template>
  <q-page class="bg-grey-1">
          ...
          <template v-else>
            <div v-if="accounts.length > 0">
              <q-list bordered class="rounded-borders">
                ...
                <q-separator />
                <q-item v-for="account in accounts" :key="account.account_id" clickable v-ripple>
                  <q-item-section>
                    <q-item-label>{{ account.account_nickname }}</q-item-label>
                    <q-item-label caption>
                      <q-badge :color="account.is_connected ? 'positive' : 'negative'"
                        :label="account.is_connected ? 'Conectado' : 'Desconectado'" />
                    </q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-btn label="Atualizar Vendas" color="primary" :loading="account.isLoading"
                      :disable="account.isLoading" @click="startOrderUpdate(account.account_id)" unelevated rounded />
                  </q-item-section>
                </q-item>
              </q-list>
            </div>
            ...
            <div v-if="sales && sales.length > 0" class="q-mt-lg">
              <q-input v-model="search" debounce="300" placeholder="Buscar por Venda ID" outlined class="q-mb-md" dense>
                  ...
                  <q-tr :props="props">
                    <q-td v-for="col in salesColumns" :key="col.name" :props="props">
                      <div class="vertical-align-top">
                        ...
                        <template v-else-if="col.name === 'quantity'">
                          <div v-for="(quantity, idx) in getTotalQuantity(props.row)" :key="idx">
                            {{ quantity }}
                          </div>
                        </template>

                        <template v-else-if="col.name === 'unit_price'">
                          <div v-for="(price, idx) in getUnitPrices(props.row)" :key="idx">
                        ...
                        <template v-else-if="col.name === 'categoria_especial'">
                          <q-chip v-for="(value, idx) in getCategoriaEspecial(props.row)" :key="idx"
                            :color="value === 'Sim' ? 'positive' : 'grey'" text-color="white" size="sm">
                            {{ value }}
                          </q-chip>
                        </template>
                        ...
                        ">
                          <div v-for="(value, idx) in getShippingDetail(props.row, col.name)" :key="idx">
                            {{ value }}
                          </div>
                        </template>

                        <template v-else-if="col.name === 'shipping_check'">
                          <q-btn flat dense round icon="local_shipping" :color="getShippingIconColor(props.row)"
                            @click="openShippingCheck(props.row)" :disable="!props.row.shipping_check">
                            <q-tooltip>
                              {{ getShippingTooltip(props.row) }}
                            </q-tooltip>
                          </q-btn>
                        </template>
                    ...
                    <q-td>
                      <q-btn-group spread flat>
                        <q-btn flat dense icon="visibility" color="primary" @click="openSaleDetails(props.row)">
                          <q-tooltip>Ver Detalhes</q-tooltip>
                        </q-btn>
                        <q-btn flat dense icon="code" color="secondary" @click="openSaleJSON(props.row)">
    ...
    <q-dialog v-model="isSaleDetailsDialogOpen" maximized transition-show="slide-up" transition-hide="slide-down">
    ...
    <q-dialog v-model="isSaleJSONDialogOpen" maximized transition-show="slide-up" transition-hide="slide-down">
      <q-card class="column full-height">
        <q-card-section class="row items-center q-pb-none">
    ...
    <q-dialog v-model="isShippingCheckDialogOpen" maximized transition-show="slide-up" transition-hide="slide-down">
      <q-card class="column full-height">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">
            Cálculo do Frete - ID: {{ selectedSale?.sale_id }}
          </div>
          <q-space />
          <q-btn icon="close" flat round dense v-close-popup />
        </q-card-section>
        ...
        <q-card-section class="col q-pt-none scroll">
          <template v-if="selectedSale?.shipping_check">
            <pre class="text-wrap">{{ JSON.stringify(selectedSale.shipping_check, null, 2) }}</pre>
          </template>
          <template v-else>
            <div class="text-center q-pa-md">
              <q-icon name="warning" size="2em" color="warning" />
              <p>Não há cálculo de frete disponível para esta venda.</p>
            </div>
          </template>
        </q-card-section>
      </q-card>
...
const isShippingCheckDialogOpen = ref(false);

const openShippingCheck = (sale) => {
  selectedSale.value = sale;
  isShippingCheckDialogOpen.value = true;
};

// Adicione junto com as outras funções
const getShippingIconColor = (sale) => {
  if (!sale.shipping_check) return 'grey';

  return sale.shipping_check.frete_com_divergencia ? 'negative' : 'positive';
};

const getShippingTooltip = (sale) => {
  if (!sale.shipping_check) return 'Frete não calculado';

  return sale.shipping_check.frete_com_divergencia
    ? 'Divergência detectada no frete'
