    ...
    <template v-else>
          ...
          <tbody>
            <tr v-for="line in lines" :key="line.id" tabindex="0" @click="$emit('select', line)" @keydown.enter="$emit('select', line)">
              ...
              <td>
                ...
                <small v-if="line.recommended_quantity == null && line.need_quantity">necessidade {{ line.need_quantity }} · confirmar ERP</small>
                <small v-else-if="line.adjusted_quantity != null">ajustado pelo operador</small>
                <small v-else>unidades sugeridas</small>
              </td>
              <td><strong>{{ formatDate(line.dispatch_by) }}</strong><small>{{ line.dispatch_by ? 'despachar até' : 'sem despacho' }}</small></td>
      ...
      <div class="fp-mobile-lines">
        <article v-for="line in lines" :key="`mobile-${line.id}`" class="fp-line-card" @click="$emit('select', line)">
          ...
          <div class="fp-line-card__quantity"><span>Enviar<strong>{{ quantity(line) }}</strong></span><span>Até<strong>{{ formatDate(line.dispatch_by) }}</strong></span></div>
...
function quantity(line) { return line.effective_quantity ?? line.recommended_quantity ?? '—' }
